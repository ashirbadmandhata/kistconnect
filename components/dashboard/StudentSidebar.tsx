...
import { BookOpen, Calendar, Home, Menu, X, ChevronLeft, LogOut, GraduationCap } from "lucide-react";
...
export function StudentSidebar() {
    ...
    const navItems = [
        ...
        { href: "/student/assignments", icon: Calendar, label: "Assignments" },
    ];
            ...
            >
                <div className="flex flex-col h-full">
                    ...
                    <div className={`p-6 border-b transition-all duration-300 ${isCollapsed ? 'lg:p-4' : ''}`}>
                        <Link href="/student" className={`flex items-center gap-2 ${isCollapsed ? 'lg:justify-center' : ''}`}>
                        ...
                        <div className={`flex items-center gap-3 p-2 rounded-lg bg-accent/50 ${isCollapsed ? 'lg:justify-center' : ''}`}>
                            ...
                            {!isCollapsed && (
                                <div className="flex-1 min-w-0 text-left">
                                    <p className="text-sm font-medium truncate">{convexUser?.name || 'Student'}</p>
